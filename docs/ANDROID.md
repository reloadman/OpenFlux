...
**Дескриптор tun нужно отдавать в Go дубликатом, иначе выключение роняет приложение.**
tun2socks открывает устройство по строке `fd://N` и на остановке делает `unix.Close(N)`
над тем самым номером, который ему передали (`core/device/fdbased/fd_unix.go`). Если это
дескриптор из `ParcelFileDescriptor`, то следом его закрывает и Kotlin — то есть один и
тот же номер закрывается дважды. К моменту второго закрытия система могла выдать этот
номер другому сокету или файлу, и приложение падает **нативно**, мимо `try/catch`, —
поэтому обёртывание `tun.close()` в `catch (_: Throwable)` от падений не спасало.

Лечится в `mobile/mobile.go`: перед `engine.Insert` дескриптор дублируется через
`unix.Dup(fd)`, и в движок уходит копия. Теперь каждая сторона закрывает собственный
номер. В `gvisor` `endpoint.Close()` пустой, так что внутри библиотеки повторного
закрытия нет — лишним было только наше.

**Счётчик трафика — единственный честный признак живого туннеля.** Статус «работает»
говорит лишь о том, что процесс поднялся и `establish()` не вернул null. Байты считает
`mobile/counters.go`: обёртка над `proxy.Proxy` оборачивает каждое соединение и складывает
прочитанное и записанное в два атомарных счётчика, которые Kotlin опрашивает раз в секунду
(`Uploaded()` / `Downloaded()`). Обёртка счётчика ставится **под** перехватчик DNS, поэтому
запросы к резолверу тоже попадают в статистику.

**Строки лога не должны менять статус.** Раньше вывод клиента слался тем же broadcast, что
и состояние, и каждая строка переключала интерфейс в «работает» — статус мигал, а понять по
нему что-либо было нельзя. Теперь в intent есть флаг `EXTRA_IS_LOG`: помеченные им сообщения
попадают только в журнал.
